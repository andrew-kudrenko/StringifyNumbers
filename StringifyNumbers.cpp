...
bool is_spacing(char symbol) 
{
	return isspace((unsigned char) symbol);
}

std::vector<std::string>* split_with_spaces(std::string text)
{
	auto accumulator = new std::vector<std::string>();

	for (int starts = 0; starts < text.size(); starts++)
	{
		for (int ends = starts + 1; ends < text.size(); ends++)
		{
			auto first = text.at(starts);
			auto last = text.at(ends);

			bool is_end = ends == text.size() - 1;

			if (is_end || (is_spacing(first) && !is_spacing(last)) || (!is_spacing(first) && is_spacing(last)))
			{
				accumulator->push_back(text.substr(starts, static_cast<size_t>(ends - starts + (is_end ? 1 : 0))));
				starts = ends - 1;
				break;
			}
		}
...
{
	auto pattern = std::regex("^(-?)(0|([1-9][0-9]*))(\\.[0-9]+)?$");
	std::string line;

	while (std::getline(*from, line))
	{
		auto parts = split_with_spaces(line);

		for (int idx = 0; idx < parts->size(); idx++)
		{
			std::string current = parts->at(idx);
			std::string word = std::regex_match(current, pattern) 
				? current + " (" + convert_word_to_number(current) + ")" 
				: current;
...
int main()
{
	...
	try
	{
		...
		{
			std::cout << "Please make sure both files are available." << std::endl;
		}
